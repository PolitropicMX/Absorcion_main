...
class Draw3d:
    ...
    def flujo(self,A,B,mass,segundero,state):# FUNCION TIPO DRAW (X,Y)
        # A,B: [x,y] 2 element array
        # mass: [x] scalar: thickness of the line
        # segundero: [x] scalar: a time-incremented variable
        # state: str: change the color depending de str inputed
        chunks = 50
        dx = (B[0]-A[0])/chunks
        dy = (B[1]-A[1])/chunks
        ...
        for i in range(chunks):
            ...
            if state == 'liq':
                ...
                pygame.draw.line(self.screen,(f,f,f),(listpositions[i,:]),(listpositions[i+1,:]),int(mass))
            elif state == 'wst':
                v = int(f/2+20)
...
#### THE MURDERER SECTION ####
    ...
    def line(self,xi,yi,xf,yf):
        pygame.draw.line(self.screen,(255,255,255),(xi,yi),(xf,yf),10)
